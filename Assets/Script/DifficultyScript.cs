...
public class DifficultyScript : MonoBehaviour {
	...
	private GameObject[] fluids = new GameObject[100];
	...
	void Difficulty(){
		...
		fluids = GameObject.FindGameObjectsWithTag ("fluid");

		//粒子の重さ
		//if (js.state != 0) {
			foreach(GameObject f in fluids){
				rigid2d = f.GetComponent<Rigidbody2D> ();
...
//			(yoi / 200 > 1) ? 
//			1 : yoi / 200;
	//	GameObject.Find("HigherWallLeft").GetComponent<Rigidbody2D>().GetComponent<BoxCollider2D>().material =
//			(PhysicMaterial)Resources.Load("Physics/BoundMaterial2");
//		GameObject.Find("HigherWallRight").GetComponent<Rigidbody2D>().GetComponent<BoxCollider2D>().material =
	//		(PhysicMaterial)Resources.Load("Physics/BoundMaterial2");
